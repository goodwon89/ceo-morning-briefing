...
def normalize_title(title: str) -> str:
    ...
    # Google News RSS 제목 끝 " - 출처명" 제거 (예: "제목 - 경향신문")
  ...
  <style>
    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
    ...
    .section-header {{
      ...
      padding: 7px 14px; margin-bottom: 12px; display: inline-flex;
    }}
    .card-bottom {{
...
<script>
  ...
  function renderNews() {{
    ...
    // 섹션 순서 고정
    const SECTION_ORDER = ['hr','ai','startup_invest','startup_launch','startup_issue'];

    container.innerHTML = filtered.map(group => {{
      // 섹션별로 기사 그룹핑
      const bySection = {{}};
      group.news.forEach(n => {{
        const s = n.section || 'etc';
        if (!bySection[s]) bySection[s] = [];
        bySection[s].push(n);
      }});
      // 섹션 순서대로 렌더링 (데이터에 있는 섹션만)
      const sectionKeys = SECTION_ORDER.filter(s => bySection[s] && bySection[s].length > 0);
      // 기타 섹션도 추가
      Object.keys(bySection).forEach(s => {{ if (!sectionKeys.includes(s)) sectionKeys.push(s); }});
        ...
        <div class="date-group">
          ...
          ${{sectionKeys.map(sKey => `
            <div class="section-group">
              <div class="section-header">${{SECTION_LABELS[sKey] || sKey}}</div>
              <div class="card-grid">
                ${{bySection[sKey].map((n, i) => `
                  <div class="card">
                    <div class="card-top">
